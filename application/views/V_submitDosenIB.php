                 ...
                 <form action="<?php echo base_url().'C_dosen/ubahStatusIB'?>" method="post">
                  ...
                  <input type="checkbox" name="simpan" value="1" required /> 
                      Dengan ini saya mengusulkan studi lanjut dan menjamin <strong>KEBENARAN dan KEASLIAN</strong> berkas yang telah dilampirkan<br><br>
                       <?php if ($data[0]==1) { ?>
                        <a href="<?php echo base_url().'C_dosen/uploadfileIB/' ?>" class="btn btn-primary" >Kembali</a>
                      <?php } else { ?>
                        <a href="<?php echo base_url().'C_dosen/uploadfileIBNon/' ?>" class="btn btn-primary" >Kembali</a> 
                      <?php } ?>
