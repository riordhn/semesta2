...
class M_penghargaan extends CI_Model{ 
	private $table = 'penghargaan';
	private $pk = 'ID_PENGHARGAAN';
	...
	public function getPenghargaan($semester,$id){
		$query=$this->db->query("Select * from penghargaan where ID_PENGHARGAAN = '$id' AND SEMESTER='$semester'");
	...
	public function upPenghargaan(){
		...
		$this->id_penghargaan = $post['idpeng'];
		$this->id_monitoring = $post['idmonitor'];
		$this->semester = $post['semester'];
		$this->nama_penghargaan = $post['nama'];
		$this->pemberi_penghargaan = $post['pemberi'];
		...
		$this->db->update($this->table, $this, array('id_penghargaan' => $post['idpeng']));
