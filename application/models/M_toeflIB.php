<?php  
class M_toeflIB extends CI_Model{ 
	private $table = 'toefl_ib';
	private $pk = 'ID_TOEFL';
	public $id_toefl;
	...
	public function getToefl($semester,$id){
		$query=$this->db->query("Select * from toefl_ib where ID_TOEFL = '$id' AND SEMESTER='$semester'");
  ...
  //       die;
	...
	{
		$post = $this->input->post();
	    $config['upload_path']          = './file/monitoringIB/toefl/';
	    ...
	    if(!empty($_FILES[$file]['name'])){
                if(!move_uploaded_file($_FILES[$file]["tmp_name"], "./file/monitoringIB/toefl/" . $config['file_name'])){
                    $this->upload->display_errors(); 
                } 
                else{
