...
class M_tugasAkhir extends CI_Model{ 
	...
	public function getTugas($semester,$id){
		$query=$this->db->query("Select * from tugas_akhir where ID_TUGAS = '$id' AND SEMESTER='$semester'");
	...
	public function updateTotalSemester($id,$semester){
		$query=$this->db->query("UPDATE monitoring SET TOTAL_SEMESTER='$semester' where ID_MONITORING = '$id'");
	}
  ...
  //       die;
	...
	{
		$post = $this->input->post();
	    $config['upload_path']          = './file/monitoring/tugas/';
	    ...
	    if(!empty($_FILES[$file]['name'])){
                if(!move_uploaded_file($_FILES[$file]["tmp_name"], "./file/monitoring/tugas/" . $config['file_name'])){
                    $this->upload->display_errors(); 
                } 
                else{
